...
class HardnessDurationBuilder(object):
    ...
    def plot_photon(self, color_classes=True):
        ...
        if color_classes:
            ...
            ax.scatter(
                self._df['T90'], self._df['photon HR'], c='#8da0cb', alpha=0.8)
    ...
    def plot_energy(self, color_classes=True):
        ...
        if color_classes:
            ...
            ax.scatter(
                self._df['T90'], self._df['energy HR'], c='#4daf4a', alpha=0.8)
    ...
    def build_sample(self, interval='fluence'):
        ...
        if interval == 'fluence':
            ...
            best_fit_model = [
                x.split('_')[-1]
                for x in self._catalog.result['flnc_best_fitting_model']
            ]
            ...
            best_fit_model = [
                x.split('_')[-1]
                for x in self._catalog.result['pflx_best_fitting_model']
            ]
        ...
        for grb, bfm in zip(grbs_to_use, best_fit_model):
            ...
            if bfm:
                ...
                self._models.append(
                    self._catalog.get_model(bfm, interval)[grb])

                self._grbs.append(grb)

    def compute_hardness_ratios(self,
                                ene_lo_min=50.,
                                ene_lo_max=300.,
                                ene_hi_min=300.,
                                ene_hi_max=1000.):
        ...
        for model in self._models:
            ...
            self._hardrats.append(
                HRCalc(model, ene_lo_min, ene_lo_max, ene_hi_min, ene_hi_max))
